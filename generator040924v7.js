...
//Nuclear2
case 'Nuclear2.json':
  const idsForNuke2 = ['metaculus-15537'];
  const latestProbabilitiesNuke2 = idsForNuke2.map(id => dataMap[id]?.latestProbability || 0).filter(x => !isNaN(x));
  const lastDayProbabilitiesNuke2 = idsForNuke2.map(id => dataMap[id]?.lastDayProbability || 0).filter(x => !isNaN(x));
  const medianLatestNuke2 = calculateMedian(latestProbabilitiesNuke2);
  const medianLastDayNuke2 = calculateMedian(lastDayProbabilitiesNuke2);
  const percentagePointDifferenceNuke2 = Math.round((medianLatestNuke2 - medianLastDayNuke2) * 100);
  const medianLatestPercentNuke2 = Math.round(medianLatestNuke2 * 100);
  headlineText = `New nuclear state ~${medianLatestPercentNuke2}% likely by 2030`;
  if (Math.abs(percentagePointDifferenceNuke2) >= 1) {
    const direction = (percentagePointDifferenceNuke2 > 0) ? 'up' : 'down';
    headlineText += `, ${direction} ${percentagePointDifferenceNuke2 >= 0 ? '+' : ''}${percentagePointDifferenceNuke2} points this month`;
  }
  break;
...
case 'IsraelHamasGazaControl.json':
  const idsForGazaControl = ['goodjudgmentopen-3069'];
    ...
    const medianLatestPercentGazaControl = Math.round(medianLatestGazaControl * 100);
      headlineText = `Gaza control: IDF ~${medianLatestPercentGazaControl}% likely to exit Gaza before October, 2024`;
  ...
  const idsForSubGazaControl = ['metaculus-19949'];
    const latestProbabilitiesSubGazaControl = idsForSubGazaControl.map(id => dataMap[id]?.latestProbability || 0).filter(x => !isNaN(x));
      ...
      const medianLatestPercentSubGazaControl = Math.round(medianLatestSubGazaControl * 100);
        subHeadlineText = `Palestinian refugees: ~${medianLatestPercentSubGazaControl}% chance ≥500k displaced from Gaza by 2026`;
...
case 'IsraelHamasHezbollah.json':
  const idsForHezbollah = ['metaculus-21320'];
  ...
  headlineText = `Israel-Hezbollah conflict: ${highestProbPercent2}% risk of ≥1k deaths in 2024`;
...
case 'IsraelHamasSaudi.json':
  ...
  headlineText = `Israel-Saudi diplomacy: ~${medianLatestPercentIsraelHamasSaudi}% chance of establishing relations in 2024`;
...
case 'IsraelHamasIran.json':
  ...
  headlineText = `Israel-Iran conflict: ~${medianLatestPercentIsraelHamasIran}% risk of ≥1k deaths in 2024`;
...
// IsraelHamasLeadership
 case 'IsraelHamasLeadership.json':
  ...
  const idsForSubLeadership = ['metaculus-19824'];
    const latestProbabilitiesSubLeadership = idsForSubLeadership.map(id => dataMap[id]?.latestProbability || 0).filter(x => !isNaN(x));
      const lastDayProbabilitiesSubLeadership = idsForSubLeadership.map(id => dataMap[id]?.lastDayProbability || 0).filter(x => !isNaN(x));
      const medianLatestSubLeadership = calculateMedian(latestProbabilitiesSubLeadership);
      const medianLastDaySubLeadership = calculateMedian(lastDayProbabilitiesSubLeadership);
      const percentagePointDifferenceSubLeadership = Math.round((medianLatestSubLeadership - medianLastDaySubLeadership) * 100);
      const medianLatestPercentSubLeadership = Math.round(medianLatestSubLeadership * 100);
        subHeadlineText = `ICC: ~${medianLatestPercentSubLeadership}% likely to charge Netanyahu by 2026`;
          if (Math.abs(percentagePointDifferenceSubLeadership) >= 1) {
            const directionSub = (percentagePointDifferenceSubLeadership > 0) ? 'up' : 'down';
            subHeadlineText += `, ${directionSub} ${percentagePointDifferenceSubLeadership >= 0 ? '+' : ''}${percentagePointDifferenceSubLeadership} points this month`;
  }
