...
function createHeadline(container, medianLatest, percentagePointDifference, dataMap, dataFileName) {
  ...
  if (headlineElement) {
    ...
    //Crimea     
     ...
     //Nuclear
     case 'Nuclear.json':
  const idsForNuclear = ['goodjudgmentopen-3017', 'metaculus-19554'];
...
//Putin
          case 'Putin.json':
          const idsForPutin = ['goodjudgmentopen-2959', 'goodjudgment-f9e7328d1c'];
          ...
          headlineText = `Putin ~${medianLatestPercentPutin}% likely to be relected with strong majority`;
    ...
    break;
      ...
      case 'Wagner.json':
        ...
        headlineText = `Post-Putin Russia: next leader ~${medianLatestPercentWagner}% likely to disapprove Ukraine invasion, ~${medianLatestPercentDemocratization}% chance of democratisation`;
        break;
...
case 'IsraelHamasGazaControl.json':
           ...
           const idsForGazaControl = ['manifold-FsaZWdy6oL6gRZJe8RGJ'];
           ...
           headlineText = `Hamas ~${medianLatestPercentGazaControl}% likely to lose power in Gaza by mid-Jan`;
