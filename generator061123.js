  ...
  if (headlineElement) {
    ...
    switch (dataFileName) {
      ...
      case 'Crimea2.json':
        ...
        const yearProb2 = Math.round(dataMap['manifold-TR1IlkXbrJ7a8iCJGD8u'].latestProbability * 100);
        //headlineText = `Crimea land bridge: ${novProb}% chance of being cut by Nov, ${yearProb}% by 2024`;
        headlineText = `Crimea land bridge: ${yearProb}%-${yearProb2}% chance of being cut`;
        break;
...
case 'Peace.json':
  ...
  if (Math.abs(percentagePointDifferencePeaceHeader) >= 1) {
    ...
    headlineText += `, ${direction} ${percentagePointDifferencePeaceHeader >= 0 ? '+' : ''}${percentagePointDifferencePeaceHeader} points this month`;
      ...
      subHeadlineText = `Russia-Ukraine war ends: ~${medianLatestPercentPeaceSubHeader}% chance`;
...
//Putin
         ...
         //sub-Putin
          ...
          break;
                 ...
                 //Territory
    case 'Territory.json':
      const idsForTerritoryHeadline = ['metaculus-10745'];
...
case 'IsraelHamasCeasefire.json':
  ...
  headlineText = `Israel-Hamas ceasefire: ~${medianLatestPercentHamasCeasefire}% chance`;
...
subHeadlineText = `Hamas surrender: ~${medianLatestPercentSubHamasCeasefire}% likely`;
...
case 'IsraelHamasHezbollah.json':
  const idsForHezbollah = ['goodjudgmentopen-3062', 'metaculus-19375', 'manifold-xpCT9MEzcBgYXIB8QO3B'];
...
case 'IsraelHamasSaudi.json':
  ...
  headlineText = `Israel-Saudi: ~${medianLatestPercentIsraelHamasSaudi}% chance of establishing relations`;
...
case 'IsraelHamasIran.json':
  const idsForIsraelHamasIran = ['metaculus-19332', 'goodjudgmentopen-3037', 'metaculus-14899', 'manifold-7kBOoB9ORfCDDqYMbXD4'];
